...
import type { OrganizationInfo } from "@/services/hero.service";

interface BentoIntroProps {
  orgInfo?:
    (OrganizationInfo & { shortName?: string; abbreviation?: string }) | null;
}

function renderOrgName(name?: string) {
  const text = (name || "Trung tâm Đổi mới Sáng tạo Gia Lai").trim();
  const lower = text.toLowerCase();

  // Highlight "Gia Lai" if present, maintaining accurate full name
  if (lower.includes("gia lai")) {
    const parts = text.split(/(gia lai)/i);
    return parts.map((part, idx) =>
      part.toLowerCase() === "gia lai" ? (
        <span key={idx} className="text-accent-red">
          {part.toUpperCase()}
        </span>
      ) : (
        part.toUpperCase()
...
function renderDescription(desc?: string) {
  if (!desc) {
    return (
      <>
        <p>
          Trung tâm Đổi mới Sáng tạo Gia Lai được phát triển theo mô hình xã hội
          hóa, hướng đến thúc đẩy chuyển đổi số, ứng dụng công nghệ và phát
          triển hệ sinh thái đổi mới sáng tạo tại Gia Lai.
        </p>
        <p>
          1. Trung tâm tập trung phát triển Trung tâm dữ liệu vùng; triển khai
          các giải pháp UAV, AI, GIS, BIM và phần mềm quản trị, phục vụ số hóa,
          quản lý, điều hành và khai thác dữ liệu trong các lĩnh vực trọng điểm.
        </p>
        <p>
          2. Trung tâm thực hiện vai trò kết nối cơ quan quản lý, doanh nghiệp,
          startup, chuyên gia, đơn vị công nghệ và nhà đầu tư, tạo môi trường
          hợp tác, thử nghiệm và đưa các giải pháp công nghệ vào ứng dụng thực
          tế.
        </p>
        <p>
          3. Trung tâm triển khai các hoạt động ươm tạo, đào tạo, tư vấn chuyển
          đổi số, hỗ trợ doanh nghiệp và phát triển nguồn nhân lực số, góp phần
          xây dựng hệ sinh thái đổi mới sáng tạo gắn với nhu cầu phát triển của
          Gia Lai.
        </p>
      </>
    );
  ...
  // If HTML format is detected
  if (desc.includes("<p>") || desc.includes("<br") || desc.includes("</div>")) {
    return (
      <div className="space-y-3" dangerouslySetInnerHTML={{ __html: desc }} />
    );
  }
...
export function BentoIntro({ orgInfo }: BentoIntroProps) {
  ...
  const eventImage =
    orgInfo?.announcement?.imageUrl || "/about-us/3A5A2610.webp";

  const shortName =
    orgInfo?.shortName || orgInfo?.abbreviation || "VDCD Gia Lai";

  return (
    <section className="select-none">
      {/* 2-Column Responsive Grid: Left = Image + Caption, Right = Content */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-x-10 lg:gap-x-12 gap-y-6 lg:gap-y-0 items-start">
        {/* ── LEFT: Base Image + Floating Loupe + Caption (lg:col-span-6) ── */}
        <div className="lg:col-span-6 flex flex-col space-y-2.5">
          ...
          {/* Event Caption directly below image */}
          <div className="w-full px-2 sm:px-4 py-2 sm:py-3 dark:bg-zinc-900/60 transition-colors duration-300">
        ...
        {/* ── RIGHT: Title + 2 Paragraphs + Corporate Metadata (lg:col-span-6) ── */}
        <div className="lg:col-span-6 flex flex-col justify-between space-y-4">
          <div className="space-y-3.5">
            ...
            <div className="space-y-3 text-zinc-650 dark:text-zinc-400 text-sm sm:text-[14.5px] leading-relaxed transition-colors duration-300">
              {renderDescription(orgInfo?.description)}
            </div>
          </div>
        </div>
      </div>
